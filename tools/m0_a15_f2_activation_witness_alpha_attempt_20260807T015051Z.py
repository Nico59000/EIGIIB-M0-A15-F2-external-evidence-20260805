...
INGRESS_RECEIPT_DIGEST = "f9a0d4548634dd3c3d95013ba8eb0f08c02b7ce6bf04f420c9a0d50cc8666289"
INGRESS_READBACK_SET_DIGEST = "3ca43a46ddd67f14380189ca73e2258b2b4f2c9945e53d0816a275eb47e6175e"
...
def verify_activation(profile: Any, envelope: Any) -> tuple[str, datetime, datetime]:
    if not isinstance(profile, dict) or set(profile) != PROFILE_KEYS:
        raise ValueError("activation-authority-profile-shape-invalid")
    if profile["role"] != "activation-authority" or profile["algorithm"] != "ed25519":
    ...
    if signature["algorithm"] != "ed25519" or signature["keyId"] != profile["keyId"]:
    ...
    return digest_hex(payload), activated_at, valid_until
...
def main() -> int:
    ...
    authority_profile = load_json(Path(args.authority_profile))
    activation_envelope = load_json(Path(args.activation_envelope))
    activation_digest, activated_at, valid_until = verify_activation(authority_profile, activation_envelope)
    ...
    witness_profile = {
        ...
        "implementation": "eigiib-m0-a15-f2-witness-alpha-v2-attempt-bound",
        "keyId": key_id,
        "algorithm": "ed25519",
    ...
    write_json(out / "activation-witness-alpha-profile.json", witness_profile)
    write_json(out / "activation-endorsement-envelope.json", endorsement_envelope)
    write_json(out / "activation-witness-alpha-record.json", record)
    write_json(out / "activation-envelope-observed.json", activation_envelope)
    write_json(out / "activation-authority-profile-observed.json", authority_profile)
    sums = [
